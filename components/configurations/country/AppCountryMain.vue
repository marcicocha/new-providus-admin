  ...
  <div class="animated fadeIn">
    ...
    <div v-if="isContextSearch" class="context-search-wrapper animated fadeIn">
      <a-row type="flex" :gutter="16">
        <a-col :span="12">
          <AppAdminInput
            v-model="contextSearchObject.state"
            label="State Name"
            border
          />
        </a-col>
        <a-col :span="12">
          <AppAdminInput
            v-model="contextSearchObject.countryName"
            label="Country Name"
            border
          />
        </a-col>
            ...
            <a-dropdown placement="bottomRight">
              ...
              <a-menu slot="overlay">
                ...
                <a-menu-item>
                  ...
                  >
                    <a-icon type="delete" />&nbsp; Delete State
                  </a>
                </a-menu-item>
              </a-menu>
            </a-dropdown>
    ...
    <AppManageCountry
      ...
      :current-country-object="currentCountryObject"
      :mode="mode"
...
import AppManageCountry from '@/components/configurations/country/AppManageCountry'
...
export default {
  ...
  methods: {
    ...
    toggleModal(mode, record) {
      if (mode) {
        this.mode = mode
        this.currentCountryObject = record
    ...
    paginationChangeHandler(pageNumber, pageSize) {
      ...
      this.fetchCountryHandler(pageObject)
    },
    async fetchCountryHandler(obj) {
      ...
      this.loading = true
      try {
        const { response } = await this.$axios.$get('/country/search', config)
    ...
    showDeleteConfirm(record) {
      ...
      this.$confirm({
        title:
          'Are you sure you want to delete this State? This action is irreversible.',
        okText: 'Yes',
        okType: 'danger',
        cancelText: 'No',
        async onOk() {
          ...
          try {
            ...
            $this.$notification.success({
              message: 'Success',
              description: `State ${record.state} Deleted Successfully `,
            })
            const contextSearchObject = {
              currentPage: 0,
            }
